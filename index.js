...
if (!CLIENT_ID) {
  console.error('Backend: ERROR - CLIENT_ID environment variable is not set!');
  console.error('Backend: Please ensure your .env file contains CLIENT_ID=your_spotify_client_id');
}
if (!CLIENT_SECRET) {
  console.error('Backend: ERROR - CLIENT_SECRET environment variable is not set!');
  console.error('Backend: Please ensure your .env file contains CLIENT_SECRET=your_spotify_client_secret');
}
...
app.get('/login', (req, res) => {
  ...
  if (!CLIENT_ID) {
    console.error('Backend: /login - CLIENT_ID is missing. Cannot proceed with authentication.');
    return res.status(500).json({ 
      error: 'Server configuration error', 
      message: 'CLIENT_ID environment variable is not set. Please check your .env file.' 
    });
  }
...
app.get('/callback', async (req, res) => {
  ...
  if (!CLIENT_ID || !CLIENT_SECRET) {
    console.error('Backend: /callback - CLIENT_ID or CLIENT_SECRET is missing.');
    return res.redirect('/#' +
      querystring.stringify({
  ...
  if (state === null) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Update tokens using the caching system (updates cache and DB async)
        console.log('Backend: /callback - Calling updateTokens after successful token exchange.');
        await updateTokens(access_token, refresh_token, data.expires_in);
        console.log('Backend: /callback - updateTokens called.');
...
// Note: Token refresh logic has been moved to middleware/spotifyAuth.js
// The caching system handles all token refresh operations automatically
